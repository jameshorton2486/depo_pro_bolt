...
function checkViteConfig() {
  ...
  const hasCOEP = /Cross-Origin-Embedder-Policy.*(require-corp|credentialless)/s.test(cfg);

  if (hasCOOP && hasCOEP) {
    pass('Cross-origin isolation headers present (COOP + COEP)');
  } else {
    fail('vite.config.ts missing COOP/COEP headers',
         'ffmpeg.wasm works best with SharedArrayBuffer — replace vite.config.ts with the version from Claude');
  }
