...
// Web Audio API loads the whole file into memory to decode. Around 500 MB
// is the practical limit in most browsers before tabs start crashing.
// Beyond this we warn the user and suggest pre-processing.
const FILE_SIZE_WARN_BYTES = 500 * 1024 * 1024; // 500 MB
const FILE_SIZE_HARD_LIMIT_BYTES = 1500 * 1024 * 1024; // 1.5 GB
...
export default function SimpleTranscribe() {
  ...
  const runPipeline = async () => {
    ...
    // Hard limit — browsers can't decode files this large in memory
    if (file.size > FILE_SIZE_HARD_LIMIT_BYTES) {
      setError(
        `File is ${formatBytes(file.size)} — too large for in-browser ` +
        `processing. Please extract the audio track in Audacity (File → ` +
        `Export → MP3) or split the recording into smaller pieces first.`,
      );
      setPhase('failed');
      return;
    }
    ...
    try {
      ...
      const compressed = await compressAudio(file, (p: CompressionProgress) => {
        setProgress(p.percent * 0.6); // compression is 0-60% of total progress
        setStatusMessage(`${capitalize(p.phase)}...`);
      });
      ...
      // Recognize common memory-pressure failures and translate to plain English
      const looksLikeMemoryError =
        /memory|allocation|encoding|EncodingError|out of memory/i.test(raw) ||
        raw.includes('decodeAudioData');
      const msg = looksLikeMemoryError
        ? `Couldn't decode the audio (${raw}). The file is likely too large or in an unsupported codec. ` +
          `Try converting it to MP3 in Audacity first — File → Export → MP3.`
        : raw;
      log(`[ERROR] ${raw}`);
      setError(msg);
      setPhase('failed');
                  ...
                  <>
                    ...
                    {file.size > FILE_SIZE_WARN_BYTES && (
                      <p className="text-[11px] text-amber-400 mt-2 leading-snug">
                        ⚠ Large file. May fail to decode in browser. Consider extracting
                        audio to MP3 in Audacity first.
                      </p>
                    )}
                  </>
                ) : (
